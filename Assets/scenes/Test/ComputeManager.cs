...
public class ComputeManager : MonoBehaviour
{
    ...
    public void Initialize(int count = 18)
    {
        ...
        VoxelColor32[] converted = new VoxelColor32[WorldManager.Instance.BlocksIds.Length];
        int cCount = 0;

        foreach (BlockData c in WorldManager.Instance.BlocksIds)
        {
            VoxelColor32 b = new VoxelColor32();
            b.color = ColorfTo32(c.color);
            b.smoothness = c.Smoothness;
            b.metallic = c.Metalic;
            converted[cCount++] = b;
        }
    ...
    public void GenerateVoxelData(Container cont, Vector3 pos)
    {
        ...
        noiseShader.SetVector("chunkPosition", cont.pos);
        ...
        voxelShader.SetVector("chunkPosition", cont.pos);
